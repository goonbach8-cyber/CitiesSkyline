...
bool City::IsShore(int x, int z) const {
    ...
    return h >= WATER_LEVEL && h < WATER_LEVEL + 0.22f;
}
...
void City::DrawTerrain() const {
    ...
    for (int z = 0; z < GRID_H; ++z) {
        for (int x = 0; x < GRID_W; ++x) {
            ...
            float h = p.y;
            if (h < WATER_LEVEL) {
                float wave = 0.018f * sinf((float)time * 1.6f + x * 0.44f + z * 0.29f);
                Color water = ((x + z) % 2 == 0) ? Color{55, 126, 166, 235} : Color{50, 119, 159, 235};
                DrawCube({p.x, WATER_LEVEL - 0.055f + wave, p.z}, CELL_SIZE * 1.01f, 0.08f, CELL_SIZE * 1.01f, water);
            } else {
                Color ground;
                if (IsShore(x, z)) {
                    ground = Color{171, 162, 122, 255};
                } else if (h > 1.75f) {
                    ground = Color{110, 121, 100, 255};
                } else {
                    float shade = Clamp((h + 0.2f) / 2.5f, 0.0f, 1.0f);
                ...
                DrawCube({p.x, h - 0.085f, p.z}, CELL_SIZE * 1.005f, 0.13f, CELL_SIZE * 1.005f, ground);
            }
        }
    }
}

void City::DrawNaturalProps() const {
    for (int z = 1; z < GRID_H - 1; ++z) {
        for (int x = 1; x < GRID_W - 1; ++x) {
            if (IsWater(x, z) || IsShore(x, z) || RoadAt(x, z) || zones_[z][x] != Zone::None) continue;
            if (BuildingIndexAt(x, z) >= 0 || ServiceIndexAt(x, z) >= 0) continue;
            float r = Hash01(x, z, 31);
            Vector3 p = GridToWorld(x, z);
            if (r < 0.060f && p.y < 1.72f) {
                float scale = 0.72f + Hash01(x, z, 44) * 0.62f;
                Color trunk{92, 70, 49, 255};
                Color leaves = Hash01(x, z, 50) < 0.5f ? Color{64, 112, 64, 255} : Color{75, 126, 70, 255};
                DrawCube({p.x + 0.10f, p.y + 0.03f, p.z + 0.10f}, 0.62f * scale, 0.025f, 0.52f * scale, Color{47, 65, 49, 80});
                DrawCylinder({p.x, p.y + 0.36f * scale, p.z}, 0.07f * scale, 0.09f * scale, 0.72f * scale, 6, trunk);
                DrawSphereEx({p.x, p.y + 1.00f * scale, p.z}, 0.43f * scale, 4, 7, leaves);
                DrawSphereEx({p.x + 0.18f * scale, p.y + 1.10f * scale, p.z - 0.08f * scale}, 0.27f * scale, 3, 6, ShiftColor(leaves, 8));
            } else if (r > 0.985f && p.y > 1.15f) {
                DrawSphereEx({p.x, p.y + 0.15f, p.z}, 0.24f, 3, 5, Color{111, 113, 105, 255});
            }
        }
    }
}
...
void City::DrawStreetLight(Vector3 p, bool alongX) const {
    ...
    DrawCylinder({p.x, p.y + 0.64f, p.z}, 0.025f, 0.035f, 1.25f, 6, pole);
    Vector3 lampPos = p;
    lampPos.y += 1.25f;
...
void City::DrawRoadTile(int x, int z) const {
    Vector3 p = GridToWorld(x, z);
    float y = p.y + 0.025f;
    ...
    if (HashCell(x, z, 90) % 17u == 0u && (horizontal != vertical)) {
...
void City::DrawRoads() const {
    for (int z = 0; z < GRID_H; ++z) {
        for (int x = 0; x < GRID_W; ++x) if (RoadAt(x, z)) DrawRoadTile(x, z);
    }
}

void City::DrawZones() const {
    for (int z = 0; z < GRID_H; ++z) {
        for (int x = 0; x < GRID_W; ++x) {
...
void City::DrawBuildings() const {
    for (const auto& b : buildings_) {
        Vector3 center = LotCenter(b.x, b.z, b.w, b.d);
        BuildingRenderer::DrawBuilding(b, center, b.w * CELL_SIZE, b.d * CELL_SIZE);
    }
}

void City::DrawServices() const {
    for (const auto& s : services_) {
...
void City::DrawTopBar() const {
    ...
    DrawText("CITY LAB v0.4", 18, 15, 25, RAYWHITE);
